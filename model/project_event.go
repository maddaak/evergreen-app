...
func (e *ProjectChangeEvent) RedactSecrets() {
	...
	modifiedWebhooks := getModifiedWebhookSecrets(e.Before.Subscriptions, e.After.Subscriptions)
...
func getModifiedWebhookSecrets(before, after []event.Subscription) map[string]struct{} {
	...
	for id := range allIDs {
		...
		if bws == nil || aws == nil || !bytes.Equal(bws.Secret, aws.Secret) || getAuthHeader(bws) != getAuthHeader(aws) {
			modified[id] = struct{}{}
		}
	}
	return modified
...
// webhookSubscribersByID returns a map of subscription ID to WebhookSubscriber for all webhook subscriptions.
