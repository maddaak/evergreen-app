...
`
	...
	s.NoError(err)
	s.NotNil(dbVersion)
	s.Require().Len(dbVersion.Errors, 3)
	s.Require().Len(dbVersion.Warnings, 0)
	s.Equal("buildvariant 'bv' must either specify run_on field or have every task specify run_on", dbVersion.Errors[0])
	s.Equal("task 'task1' does not contain any commands", dbVersion.Errors[1])
	s.Equal("task 'task2' does not contain any commands", dbVersion.Errors[2])
